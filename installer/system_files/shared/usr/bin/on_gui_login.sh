...
if [[ ! -d /sys/firmware/efi ]]; then
    yad --undecorated --on-top --timeout=0 --button=Shutdown:0 \
        --text="YaguareteOS does not support CSM/Legacy Boot. Please boot into your UEFI/BIOS settings, disable CSM/Legacy Mode, and reboot." || true
...
serve_docs
   text="You need <b>at least 8GB of system memory</b>  to install YaguareteOS. \n\n Installation with 4GB or less memory will likely fail.\n\nDetected amount of memory: $gb_memory\n\n Please read <a href=\"https://github.com/lobinuxsoft/yaguarete_os/wiki\">here</a> about minimum system requirements for YaguareteOS."
...
welcome_dialog() {
    ...
    _RETVAL=0
    local welcome_text="
Welcome to the Live ISO for YaguareteOS\\!
...
nvidia_hardware_helper() {
    ...
    if [[ -f "/usr/libexec/bazzite_detect_nvidia_support_status" ]]; then
        ...
        #user facing text
        title="YaguareteOS Hardware Helper"
        image_detected="Detected YaguareteOS version: $(echo "$image_name" |  cut -d '/' -f3)\n\n"
        qrencode -o "\$SUPPORT_QR" "https://github.com/lobinuxsoft/yaguarete_os/discussions"
        support="\n\n\nPlease join our <a href=\"https://github.com/lobinuxsoft/yaguarete_os/discussions\"><b>Discord Server</b></a> (scan the QR code) for support."
        heading_nvidia_deck="<b>STEAM GAMING MODE IN BETA ON NVIDIA HARDWARE</b>\n"
        detected_nvidia_deck="WARNING: Nvidia GPU Support in Steam Gaming mode and on HTPCs is available as a beta with known issues that <b>cannot be fixed</b> by YaguareteOS.\n\n"
        ...
        recommend_unsupported="Please read our <a href=\"https://github.com/lobinuxsoft/yaguarete_os/wiki\"><b>documentation</b></a> for more information.\n"
        heading_unknown="<b>Unknown Graphics Card</b>\n"
        detected_unknown="We could not identify your NVIDIA graphics card.\n"
        recommend_unknown="It is not recommended to install YaguareteOS as we cannot guarantee your hardware will work."
        heading_wrong_image="<b>WRONG IMAGE DETECTED</b>\n"
        detected_wrong_image="Your $support_status NVIDIA graphics card needs a different version of YaguareteOS.\n"
        ...
        button1="I KNOW WHAT I AM DOING. Install YaguareteOS Anyway:0"
        button2="Power Off:1"
...
for device in "${!mount[@]}"; do
    ...
    msg=$(sudo -E unshare -m sh -c '
        ...
        for dir in "$mnt"/EFI/*; do
            ...
            grub=("$dir"/grub*.efi)
            (( ! ${#grub[@]} )) && continue
            echo "The GRUB bootloader seems to be installed on $device at ${dir#$mnt}\nYaguareteOS <a href=\"https://github.com/lobinuxsoft/yaguarete_os/wiki\">does not support dual boot with any other Linux installation.</a> \nInstalls to this disk that attempt to reuse this EFI partition will fail.\nEither YaguareteOS must be installed to a different disk, or this partition or boot loader must be removed.\n\nPlease see the <a href=\"https://github.com/lobinuxsoft/yaguarete_os/wiki\">documentation</a> for instructions.\n"
        done
    ' || true)
    [ "$msg" ] || continue
    serve_docs
