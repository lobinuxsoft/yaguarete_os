...
if mountpoint -q /usr/lib/containers/storage; then
    # Load image from host container storage if possible
...
if [[ "$desktop_env" == "gnome" ]]; then
    ...
    cp -a /src/system_files/gnome/. / 2>/dev/null || echo "no gnome system_files to copy"
elif [[ "$desktop_env" == "kde" ]]; then
    echo "Copying KDE-specific system files..."
    cp -a /src/system_files/kde/. / 2>/dev/null || echo "no kde system_files to copy"
fi
...
# Copy overrides (applied on top of everything else)
echo "Copying overrides of system files..."
cp -af /src/system_files/overrides/. / 2>/dev/null || echo "no overrides to copy"
...
# Set the timezone to UTC for the live ISO (installed system gets es_AR via kickstart)
rm -f /etc/localtime
systemd-firstboot --timezone UTC

# / in a booted live ISO is an overlayfs with upperdir pointed somewhere under /run.
# /var/tmp ends up on /run (tmpfs, small). ostree needs a lot of space there for temp files.
# Mount a larger tmpfs to /var/tmp at boot time.
rm -rf /var/tmp
mkdir /var/tmp
...
# Mount /var/lib/flatpak as readonly so files don't get tainted during install
...
# Copy iso config for image-builder
