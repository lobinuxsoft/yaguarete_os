...
A user unit checks the result at every login and says so in the journal: `yaguarete-tdp-verify` reads the same `TdpLimitMax` Steam reads and stays quiet unless it is zero, in which case it prints which of the two defects is back. The chain fails silently otherwise — the only symptom is a control that is not in the menu — and it *will* fail: the day upstream corrects the class mapping, the quirk stops helping and starts hurting.
